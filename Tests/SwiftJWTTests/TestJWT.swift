...
let rsaJWTKidEncoder = JWTEncoder(keyIDToSigner: { kid in jwtSigners[kid] })
let rsaJWTKidDecoder = JWTDecoder(keyIDToVerifier: { kid in jwtVerifiers[kid] })
...
//
	...
	func signAndVerify(signer: JWTSigner, verifier: JWTVerifier) throws {
		...
		jwt.claims.nbf = Date(timeIntervalSince1970: 1_485_949_565.58463)
	...
	func check(jwt: JWT<TestClaims>, algorithm: String) {
		...
		XCTAssertEqual(jwt.claims.exp, Date(timeIntervalSince1970: 2_485_949_565.58463), "Wrong .exp in decoded")
		XCTAssertEqual(jwt.claims.iat, Date(timeIntervalSince1970: 1_485_949_565.58463), "Wrong .iat in decoded")
		XCTAssertEqual(jwt.claims.nbf, Date(timeIntervalSince1970: 1_485_949_565.58463), "Wrong .nbf in decoded")
	}

	func checkMicroProfile(jwt: JWT<MicroProfile>, algorithm _: String) {
